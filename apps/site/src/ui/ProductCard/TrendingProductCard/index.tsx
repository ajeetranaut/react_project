...
export const TrendingProductCard = ({ data }: TrendingProductCardProps) => {
  return (
    <div className="p-5 border border-neutral-content rounded-2xl w-full">
      ...
      {data?.name ? (
        <Link href={`/shop/product/${data?.slug}`}>
          <h4 className="text-xl font-normal text-secondary mt-5 hover:text-primary transition hover:duration-700 line-clamp-2">
            {data.name}
          </h4>
        </Link>
      ) : (
...
TrendingProductCard.defaultProps = defaultData;
